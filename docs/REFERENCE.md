...
Agent Control provides a control layer that sits between your AI agents and the outside world. It evaluates inputs and outputs against configurable rules, blocking harmful content, prompt injections, PII leakage, and other risks.
...
- Reuse controls across multiple agents
- Add or remove controls from agents at runtime
...
2. **Control Loading**: Server returns the agent's assigned controls
...
| `agent_control.add_control_to_agent()` | Add a control to an agent |
| `agent_control.remove_control_from_agent()` | Remove a control from an agent |
