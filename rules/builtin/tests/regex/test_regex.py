"""Tests for the regex rule and its config validation."""

from __future__ import annotations

import pytest
from agent_control_rules.regex.config import RegexRuleConfig
from agent_control_rules.regex.rule import RegexRule
...
class TestRegexConfig:
    ...
    def test_valid_pattern_accepted(self):
        config = RegexRuleConfig(pattern=r"\d{3}-\d{2}-\d{4}")
    ...
    def test_empty_pattern_accepted(self):
        ...
        config = RegexRuleConfig(pattern="")
        assert config.pattern == ""

    def test_invalid_pattern_rejected(self):
        with pytest.raises(ValueError, match="Invalid regex pattern"):
            RegexRuleConfig(pattern="[invalid(regex")

    def test_flags_default_to_none(self):
        config = RegexRuleConfig(pattern=r"\d+")
...
class TestRegexRule:
    ...
    async def test_match_returns_matched_true(self):
        rule = RegexRule.from_dict({"pattern": r"\d{3}-\d{4}"})

        result = await rule.evaluate("call 555-1234 today")
    ...
    async def test_none_data_returns_no_data_message(self):
        rule = RegexRule.from_dict({"pattern": r".*"})
    ...
    async def test_non_string_data_is_coerced(self):
        ...
        rule = RegexRule.from_dict({"pattern": r"^42$"})

        result = await rule.evaluate(42)
    ...
    async def test_unknown_flag_is_ignored(self):
        ...
        result = await rule.evaluate("xyz")
    ...
    async def test_case_sensitive_by_default(self):
        rule = RegexRule.from_dict({"pattern": "Secret"})
