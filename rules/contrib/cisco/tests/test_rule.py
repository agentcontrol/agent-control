...
import pytest
import agent_control_rule_cisco.ai_defense.rule as cisco_rule_module
from pydantic import ValidationError

from agent_control_rule_cisco.ai_defense import (
    CiscoAIDefenseRule,
    CiscoAIDefenseConfig,
)
from agent_control_rule_cisco.ai_defense.client import AIDefenseClient
...
def test_metadata_version_matches_distribution_version() -> None:
    assert CiscoAIDefenseRule.metadata.version == version("agent-control-rule-cisco")
...
def test_metadata_version_falls_back_without_distribution(
    ...
    monkeypatch.setattr(cisco_rule_module, "version", _raise_not_found)

    assert cisco_rule_module._resolve_package_version() == "0.0.0.dev"
