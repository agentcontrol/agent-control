...
def _resolve_package_version() -> str:
    ...
    try:
        return version("agent-control-rule-cisco")
...
@register_rule
class CiscoAIDefenseRule(Rule[CiscoAIDefenseConfig]):
    """Cisco AI Defense rule.

    Maps InspectResponse.is_safe to RuleResult.matched.
    """
    ...
    def is_available(cls) -> bool:
        """Rule is available only if httpx dependency exists."""
    ...
    async def evaluate(self, data: Any) -> RuleResult:  # noqa: D401
        # Null input: do not call external service; treat as no data
        if data is None:
            return RuleResult(matched=False, confidence=1.0, message="No data")
        ...
        if not messages:
            return RuleResult(matched=False, confidence=1.0, message="No data to inspect")
        ...
        try:
            ...
            # If no boolean is present, consider it a rule error
