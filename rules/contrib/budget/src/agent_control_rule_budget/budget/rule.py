"""Budget rule -- tracks cumulative LLM token/cost usage.

Deterministic rule: confidence is always 1.0, matched is True when
any configured limit is exceeded. Utilization ratio and spend breakdown
are returned in result metadata, not in confidence.

The rule is stateless. Budget state lives in a module-level store
registry, independent of the rule instance cache in _factory.py.
...
from .config import BudgetRuleConfig, ModelPricing
...
def _resolve_package_version() -> str:
    ...
    try:
        return version("agent-control-rule-budget")
...
#
# Decoupled from the rule instance cache so that LRU eviction in
...
def get_or_create_store(config: BudgetRuleConfig) -> BudgetStore:
...
@register_rule
class BudgetRule(Rule[BudgetRuleConfig]):
    """Tracks cumulative LLM token and cost usage per scope and time window.

    Deterministic rule: matched=True when any configured limit is
    exceeded, confidence=1.0 always.

    The rule is stateless. Budget state is managed by a module-level
    store registry (get_or_create_store), not by the rule instance.
    """
    ...
    async def evaluate(self, data: Any) -> RuleResult:
        ...
        if not model_known:
            ...
            if has_matching_cost_rule:
                ...
                logger.warning(
                    "Budget rule: %s, treating cost as 0 "
