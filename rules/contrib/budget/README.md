# agent-control-rule-budget

Budget rule for agent-control that tracks cumulative LLM token and cost usage per scope and time window.

## Install

```bash
pip install "agent-control-rules[budget]"
```

Fallback direct wheel install:

```bash
pip install agent-control-rule-budget
```

For local development:

```bash
uv pip install -e rules/contrib/budget
```

## Quickstart

```python
from agent_control_rule_budget.budget import (
    BudgetRuleConfig,
    BudgetLimitRule,
    ModelPricing,
)
...
The rule reads token usage from standard fields such as `usage.input_tokens` and `usage.output_tokens`. Configure `token_path` only when your event shape uses a custom location.
...
Rules with the same `budget_id` share accumulated spend and token totals across all rule instances. Each rule still evaluates using its own configured rules -- the shared state is the bucket (the rolling sum), not the rule set. Rules with different `budget_id` values are fully isolated.
...
Use two rules when cost and token ceilings need independent control records or different `budget_id` pools:

```python
cost_config = BudgetRuleConfig(
...
This pattern lets cost and token budgets reset, alert, and roll out independently. A single rule can also contain both rules when one shared pool and one control result are sufficient.
