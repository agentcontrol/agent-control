"""Tests for the budget rule (contrib).
...
import agent_control_rule_budget.budget.rule as budget_rule_module
...
from agent_control_rule_budget.budget.rule import (
    BudgetRule,
    _extract_tokens,
    clear_budget_stores,
    get_or_create_store,
)
from agent_control_rule_budget.budget.memory_store import (
    InMemoryBudgetStore,
    _build_scope_key,
...
def test_metadata_version_matches_distribution_version() -> None:
    assert BudgetRule.metadata.version == version("agent-control-rule-budget")
...
def test_metadata_version_falls_back_without_distribution(
    ...
    monkeypatch.setattr(budget_rule_module, "version", _raise_not_found)

    assert budget_rule_module._resolve_package_version() == "0.0.0.dev"
...
class TestUtilities:
    ...
    def test_parse_period_key_valid(self) -> None:
        ...
        from agent_control_rule_budget.budget.memory_store import _parse_period_key
...
class TestModelPricing:
    def test_model_pricing_validation_requires_pricing_for_cost_rules(self) -> None:
        ...
        with pytest.raises(ValidationError, match="pricing is required"):
            BudgetRuleConfig(limits=[BudgetLimitRule(limit=100)])

    def test_model_pricing_token_rule_no_pricing_ok(self) -> None:
        # Given: a token-only rule without pricing
        # When: config is created
        config = BudgetRuleConfig(limits=[BudgetLimitRule(limit=100, limit_unit="tokens")])
...
# ---------------------------------------------------------------------------
# BudgetRule integration
# ---------------------------------------------------------------------------


class TestBudgetRule:
    def _make_rule(self, **kwargs: Any) -> BudgetRule:
        config = BudgetRuleConfig(**kwargs)
        return BudgetRule(config)

    @pytest.mark.asyncio
    async def test_single_call_under_budget(self) -> None:
        # Given: rule with 1000-token limit (token-only, no pricing needed)
        ev = self._make_rule(limits=[{"limit": 1000, "limit_unit": "tokens"}])
    ...
    async def test_accumulate_past_budget(self) -> None:
        # Given: rule with 50-cent limit and pricing table
        ev = self._make_rule(
            limits=[{"limit": 50}],
    ...
    async def test_token_only_limit(self) -> None:
        ...
        ev = self._make_rule(limits=[{"limit": 500, "limit_unit": "tokens"}])
    ...
    async def test_no_data_returns_not_matched(self) -> None:
        # Given: rule / When: None data / Then: not matched
        ev = self._make_rule(limits=[{"limit": 1000}], pricing={}, model_path="model")
        result = await ev.evaluate(None)
        assert result.matched is False

    @pytest.mark.asyncio
    async def test_confidence_always_one(self) -> None:
        # Given: rule with 1000-cent limit and pricing table
        ev = self._make_rule(
            limits=[{"limit": 1000}],
            pricing={"gpt-4": {"input_per_1k": 200.0, "output_per_1k": 0.0}},
            model_path="model",
    ...
    async def test_cost_computed_from_pricing_table(self) -> None:
        # Given: rule with pricing table and 100-cent cost limit
        ev = self._make_rule(
            limits=[{"limit": 100}],
            pricing={"gpt-4": {"input_per_1k": 30.0, "output_per_1k": 60.0}},
            model_path="model",
    ...
    async def test_unknown_model_cost_zero(self) -> None:
        # Given: rule with warn mode and data from an unknown model
        ev = self._make_rule(
            limits=[{"limit": 100}],
            pricing={"gpt-4": {"input_per_1k": 30.0, "output_per_1k": 60.0}},
            model_path="model",
    ...
    async def test_small_cost_no_overcount(self) -> None:
        # Given: rule with 1-cent limit, pricing yields 0.003 cents per call
        ev = self._make_rule(
            limits=[{"limit": 1}],
...
class TestBudgetIdSemantics:
    ...
    async def test_same_budget_id_shares_store(self) -> None:
        # Given: two rules with the same budget_id
        config1 = BudgetRuleConfig(
            limits=[{"limit": 100}],
            budget_id="shared",
            pricing={"gpt-4": {"input_per_1k": 100.0, "output_per_1k": 0.0}},
            model_path="model",
        )
        config2 = BudgetRuleConfig(
            limits=[{"limit": 100}],
            budget_id="shared",
            pricing={"gpt-4": {"input_per_1k": 100.0, "output_per_1k": 0.0}},
            model_path="model",
        )
        ev1 = BudgetRule(config1)
        ev2 = BudgetRule(config2)
        step = {"model": "gpt-4", "usage": {"input_tokens": 500, "output_tokens": 0}}

        # When: each rule records a 50-cent call
    ...
    async def test_different_budget_id_isolates_store(self) -> None:
        # Given: two rules with different budget_id values
        config1 = BudgetRuleConfig(
            limits=[{"limit": 100}],
            budget_id="pool-a",
            pricing={"gpt-4": {"input_per_1k": 100.0, "output_per_1k": 0.0}},
...
class TestUnknownModelBehavior:
    ...
    async def test_unknown_model_block_default(self) -> None:
        ...
        # Then: the rule fails closed and reports the unknown model
        assert result.matched is True
        assert result.metadata is not None
    ...
    async def test_unknown_model_warn(self) -> None:
        ...
        # Then: the rule treats cost as 0 and does not block
        assert result.matched is False
        assert result.metadata is not None
        assert result.metadata["cost"] == 0.0
...
class TestStoreRegistry:
    def test_same_config_returns_same_store(self) -> None:
        ...
        config = BudgetRuleConfig(limits=[{"limit": 1000}], pricing={}, model_path="model")
    ...
    @pytest.mark.asyncio
    async def test_rule_uses_registry(self) -> None:
        # Given: two rules with same config
        config = BudgetRuleConfig(
            limits=[{"limit": 100}],
            pricing={"gpt-4": {"input_per_1k": 100.0, "output_per_1k": 0.0}},
            model_path="model",
        )
        ev1 = BudgetRule(config)
        ev2 = BudgetRule(config)
    ...
    async def test_same_budget_id_shares_buckets_but_not_rules(self) -> None:
        ...
        # When: the first rule records 60 cents, then the second records
        # another 60 cents into the same budget bucket
        first = await ev1.evaluate(step)
        second = await ev2.evaluate(step)

        # Then: the second rule sees shared bucket state (120 cents) but
...
class TestBudgetAdversarial:
    ...
    def test_extract_by_path_rejects_dunder(self) -> None:
        from agent_control_rule_budget.budget.rule import _extract_by_path
...
class TestStoreRegistryRobustness:
    ...
    @pytest.mark.asyncio
    async def test_rule_cache_eviction_preserves_budget_state(self) -> None:
        # Given: rule that has recorded usage
        from agent_control_rules._factory import (
            clear_rule_cache,
        )
        ...
        ev = BudgetRule(config)
        step = {"model": "gpt-4", "usage": {"input_tokens": 500, "output_tokens": 0}}
        await ev.evaluate(step)

        # When: simulate LRU eviction by clearing the rule cache
        clear_rule_cache()

        # Then: budget state survives (stored in module-level registry, not on rule)
        ev2 = BudgetRule(config)
        result = await ev2.evaluate(step)

        # 500 tokens * 100 cents/1k = 50.0 cents per call.
        # Two calls = 100.0 cents total. limit=1000, so not exceeded.
        # Key assertion: state IS preserved across rule re-creation.
        assert result.metadata is not None
...
class TestEstimateCostEdgeCases:
    def test_nan_rate_returns_zero(self) -> None:
        from agent_control_rule_budget.budget.rule import _estimate_cost
...
class TestNestedModelPath:
    ...
    async def test_nested_model_path(self) -> None:
        # Given: rule with nested model_path
        ev = BudgetRule(
            BudgetRuleConfig(
                limits=[{"limit": 1000}],
                pricing={"gpt-4": {"input_per_1k": 100.0, "output_per_1k": 0.0}},
...
class TestScopedUnknownModelBlock:
    ...
    async def test_unknown_model_not_blocked_when_only_token_rule_matches(self) -> None:
        # Given: rule with a token-only rule for scope A and a cost rule for scope B
        config = BudgetRuleConfig(
            limits=[
...
class TestBudgetStoreABC:
    ...
    def test_subclass_without_override_accepted_at_class_creation(self) -> None:
        ...
        from agent_control_rule_budget.budget.store import BudgetStore
