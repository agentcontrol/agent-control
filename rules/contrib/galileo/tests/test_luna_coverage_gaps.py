"""Targeted tests filling coverage gaps in luna/rule.py and luna/client.py.

These tests cover the small utility functions and rare branches that the
integration-style tests in ``test_luna_rule.py`` skip past.
"""
...
class TestCoercePayloadText:
    ...
    def test_none_returns_none(self):
        from agent_control_rule_galileo.luna.rule import _coerce_payload_text
...
class TestExtractDictText:
    ...
    def test_missing_key_returns_none(self):
        from agent_control_rule_galileo.luna.rule import _extract_dict_text
...
class TestContains:
    ...
    def test_none_threshold_is_no_match(self):
        from agent_control_rule_galileo.luna.rule import _contains
...
class TestConfidenceFromScore:
    ...
    def test_true_bool_maps_to_one(self):
        from agent_control_rule_galileo.luna.rule import _confidence_from_score
...
# =============================================================================
# luna/rule.py: _score_matches operator branches
# =============================================================================


@pytest.fixture
def luna_rule(monkeypatch):
    """A ready-to-use LunaRule instance with auth env wired up."""
    monkeypatch.setenv("GALILEO_API_KEY", "test-key")
    from agent_control_rule_galileo.luna import LunaRule

    return LunaRule.from_dict(
...
class TestScoreMatchesOperators:
    ...
    def test_any_truthy_score_matches(self, monkeypatch):
        rule = self._make("any", 0.5, monkeypatch)
        assert rule._score_matches(1) is True
        assert rule._score_matches(0) is False

    def test_eq_matches_threshold(self, monkeypatch):
        rule = self._make("eq", "flagged", monkeypatch)
        assert rule._score_matches("flagged") is True
        assert rule._score_matches("safe") is False

    def test_ne_matches_when_different(self, monkeypatch):
        rule = self._make("ne", "flagged", monkeypatch)
        assert rule._score_matches("safe") is True
        assert rule._score_matches("flagged") is False

    def test_contains_matches_substring(self, monkeypatch):
        rule = self._make("contains", "flag", monkeypatch)
        assert rule._score_matches("flagged") is True
        assert rule._score_matches("clean") is False
    ...
    def test_numeric_operators_all_branches(self, monkeypatch):
        for op, expectations in [
            ...
            rule = self._make(op, 0.5, monkeypatch)
            for score, expected in expectations:
                assert rule._score_matches(score) is expected, (op, score)

    def test_numeric_operator_rejects_non_numeric_score(self, monkeypatch):
        rule = self._make("gte", 0.5, monkeypatch)
        with pytest.raises(ValueError, match="not numeric"):
            rule._score_matches("not-a-number")


# =============================================================================
# luna/rule.py: payload preparation + aclose
# =============================================================================
...
class TestPreparePayload:
    ...
    def test_scalar_routed_to_input_when_label_lacks_output(self, monkeypatch):
        ...
        rule = LunaRule.from_dict({"scorer_label": "toxicity", "threshold": 0.5})

        input_text, output_text = rule._prepare_payload("hello")
    ...
    def test_structured_payload_uses_input_output_keys_over_payload_field(self, monkeypatch):
        ...
        input_text, output_text = rule._prepare_payload(
...
@pytest.mark.asyncio
async def test_rule_aclose_closes_underlying_client(monkeypatch):
    ...
    assert rule._client is fake


@pytest.mark.asyncio
async def test_rule_handles_non_success_status(monkeypatch):
    """A non-success status from the scorer must surface as an error result."""
    monkeypatch.setenv("GALILEO_API_KEY", "test-key")
    from agent_control_rule_galileo.luna import LunaRule, ScorerInvokeResponse
    from agent_control_rule_galileo.luna.client import GalileoLunaClient
...
# =============================================================================
# luna/rule.py: package version fallback
# =============================================================================
...
def test_resolve_package_version_falls_back_when_metadata_missing():
    ...
    from agent_control_rule_galileo.luna import rule as rule_module

    with patch.object(rule_module, "version", side_effect=PackageNotFoundError):
        result = rule_module._resolve_package_version()
...
class TestAsFloatOrNone:
    ...
    def test_returns_none_for_bool(self):
        from agent_control_rule_galileo.luna.client import _as_float_or_none
...
class TestHasValue:
    ...
    def test_none_is_empty(self):
        from agent_control_rule_galileo.luna.client import _has_value
