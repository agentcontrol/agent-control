"""Tests for the direct Galileo Luna rule and client."""
...
class TestLunaRuleConfig:
    """Tests for direct Luna rule configuration."""

    def test_config_accepts_direct_scorer_fields(self) -> None:
        from agent_control_rule_galileo.luna import LunaRuleConfig

        # Given: a direct scorer config with local thresholding
        config = LunaRuleConfig(
    ...
    def test_numeric_operator_requires_numeric_threshold(self) -> None:
        ...
        with pytest.raises(ValidationError, match="numeric threshold"):
            LunaRuleConfig(scorer_label="toxicity", threshold="high", operator="gte")
...
class TestGalileoLunaClient:
    ...
    def test_scorer_invoke_request_matches_api_schema_shape(self) -> None:
        from agent_control_rule_galileo.luna import ScorerInvokeInputs, ScorerInvokeRequest
    ...
    def test_scorer_invoke_request_requires_input_or_output(self, empty_value: object) -> None:
        from agent_control_rule_galileo.luna import ScorerInvokeRequest
    ...
    def test_scorer_invoke_response_matches_api_schema_shape(self) -> None:
        from agent_control_rule_galileo.luna import ScorerInvokeResponse
    ...
    def test_client_uses_protect_api_url_derivation(self) -> None:
        from agent_control_rule_galileo.luna import GalileoLunaClient
    ...
    async def test_client_applies_ca_bundle_and_connection_limits(self) -> None:
        ...
        with patch(
            "agent_control_rule_galileo.luna.client.httpx.AsyncClient", recording_client
        ):
            try:
    ...
    async def test_client_pool_size_rotates_across_http_clients(self) -> None:
        import agent_control_rule_galileo.luna.client as luna_client_module
        from agent_control_rule_galileo.luna import GalileoLunaClient
...
class TestLunaRule:
    """Tests for direct Luna rule behavior."""

    @patch.dict(os.environ, {"GALILEO_API_KEY": "test-key"})
    def test_rule_metadata(self) -> None:
        from agent_control_rule_galileo.luna import LunaRule

        assert LunaRule.metadata.name == "galileo.luna"
        assert LunaRule.metadata.requires_api_key is True

    @patch.dict(os.environ, {}, clear=True)
    def test_rule_init_without_auth_raises(self) -> None:
        from agent_control_rule_galileo.luna import LunaRule

        with pytest.raises(ValueError, match="GALILEO_API_SECRET_KEY or GALILEO_API_KEY"):
            LunaRule.from_dict({"scorer_label": "toxicity", "threshold": 0.5})

    @patch.dict(os.environ, {"GALILEO_API_SECRET_KEY": "test-secret"}, clear=True)
    def test_rule_init_accepts_api_secret(self) -> None:
        from agent_control_rule_galileo.luna import LunaRule
        ...
        assert rule.config.scorer_label == "toxicity"

    @patch.dict(os.environ, {"GALILEO_API_KEY": "test-key"})
    @pytest.mark.asyncio
    async def test_rule_applies_threshold_locally_to_raw_score(self) -> None:
        ...
        # Given: a direct Luna rule and a raw successful scorer response
        rule = LunaRule.from_dict(
            {
                "scorer_label": "toxicity",
                "threshold": 0.7,
    ...
    @patch.dict(os.environ, {"GALILEO_API_KEY": "test-key"})
    @pytest.mark.asyncio
    async def test_rule_returns_non_match_below_threshold(self) -> None:
        from agent_control_rule_galileo.luna import LunaRule, ScorerInvokeResponse
        from agent_control_rule_galileo.luna.client import GalileoLunaClient
    ...
    async def test_rule_does_not_call_api_for_empty_data(self, data: str) -> None:
        from agent_control_rule_galileo.luna import LunaRule
        from agent_control_rule_galileo.luna.client import GalileoLunaClient

        # Given: a rule and empty selected data
        rule = LunaRule.from_dict({"scorer_label": "toxicity", "threshold": 0.5})

        with patch.object(GalileoLunaClient, "invoke", new_callable=AsyncMock) as mock_invoke:
            # When: evaluating empty data
            result = await rule.evaluate(data)
    ...
    @patch.dict(os.environ, {"GALILEO_API_KEY": "test-key"})
    @pytest.mark.asyncio
    async def test_rule_fail_open_sets_error(self) -> None:
        from agent_control_rule_galileo.luna import LunaRule
        from agent_control_rule_galileo.luna.client import GalileoLunaClient
    ...
    @patch.dict(os.environ, {"GALILEO_API_KEY": "test-key"})
    @pytest.mark.asyncio
    async def test_rule_error_metadata_includes_http_status_context(self) -> None:
        from agent_control_rule_galileo.luna import LunaRule
        from agent_control_rule_galileo.luna.client import GalileoLunaClient
