...
test.describe('Agent Detail Page', () => {
  ...
  test('full JSON editor omits null condition fields from display', async ({
    mockedPage,
  }) => {
    const noisyCompositeControl: Control = {
      id: 88,
      name: 'Public Slack Mode',
      control: {
        description: 'Composite control with nullable transport fields',
        enabled: true,
        execution: 'server',
              ...
              or: [
                {
                  ...
                  not: {
                    ...
                    evaluator: {
                      name: 'regex',
                      config: {
                        pattern: '(^|\\.)(read|memory_search|memory_get)$',
                      },
                    },
                    and: null,
                    or: null,
    ...
    const noisyControls: AgentControlsResponse = {
      controls: [noisyCompositeControl],
    };

    await mockRoutes.agent(mockedPage, {
      controls: { data: noisyControls },
    });
    await mockedPage.route('**/api/v1/controls/88/data', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ data: noisyCompositeControl.control }),
      });
    });

    await mockedPage.goto(
      getAgentControlsUrl({ modal: 'edit', controlId: 88 })
    );
    ...
    await expect(modal).not.toContainText('"selector": null');
    await expect(modal).not.toContainText('"evaluator": null');
    await expect(modal).not.toContainText('"and": null');
    await expect(modal).not.toContainText('"or": null');
    await expect(modal).not.toContainText('"not": null');
    await expect(modal).toContainText('"context.channel.scope"');
