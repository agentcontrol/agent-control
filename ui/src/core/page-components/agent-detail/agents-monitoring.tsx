...
import type { TimeRangeType,TimeRangeValue } from "@rungalileo/jupiter-ds";
...
interface AgentsMonitoringProps {
  agentUuid: string;
}

// Map Jupiter DS TimeRangeType to our TimeRange
const mapTimeRangeTypeToTimeRange = (type: TimeRangeType): TimeRange => {
...
export function AgentsMonitoring({ agentUuid }: AgentsMonitoringProps) {
  // Store TimeRangeValue directly to preserve the selected option
  const [timeRangeValue, setTimeRangeValue] = useState<TimeRangeValue>({
    type: "lastHour", // Default to 1h
  });
  ...
  const {
    ...
    isLoading,
    error,
  } = useAgentStats(agentUuid, apiTimeRange, {
