/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v4-mini";
import { safeParse } from "../lib/schemas.js";
import { Result as SafeParseResult } from "../types/fp.js";
import * as types from "../types/primitives.js";
import { SDKValidationError } from "./errors/sdk-validation-error.js";

/**
 * Response from control metadata update.
 */
export type PatchControlResponse = {
  /**
   * Current enabled status (if control has data configured)
   */
  enabled?: boolean | null | undefined;
  /**
   * Current control name (may have changed)
   */
  name: string;
  /**
   * Whether the update succeeded
   */
  success: boolean;
};

/** @internal */
export const PatchControlResponse$inboundSchema: z.ZodMiniType<
  PatchControlResponse,
  unknown
> = z.object({
  enabled: z.optional(z.nullable(types.boolean())),
  name: types.string(),
  success: types.boolean(),
});

export function patchControlResponseFromJSON(
  jsonString: string,
): SafeParseResult<PatchControlResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => PatchControlResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'PatchControlResponse' from JSON`,
  );
}
