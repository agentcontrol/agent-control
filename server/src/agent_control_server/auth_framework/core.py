...
def set_authorizer(authorizer: RequestAuthorizer | None) -> None:
    """Install the process-wide authorizer.

    Called once at startup from :mod:`auth_framework.config`. Tests use
    it via fixtures to swap in deterministic providers.
    """
    global _authorizer
    _authorizer = authorizer


def get_authorizer() -> RequestAuthorizer:
    """Return the installed authorizer or raise if none is set."""
...
def require_operation(
    ...
    *,
    ...
    The dependency consults the installed authorizer and returns the
    resulting :class:`Principal` so endpoints can read the resolved
    ``namespace_key`` without re-deriving it.
    ...
    async def dependency(request: Request) -> Principal:
        ...
        if context_builder is not None:
            ...
            if inspect.isawaitable(built):
                built = await built
            context = built
        return await get_authorizer().authorize(request, operation, context)
