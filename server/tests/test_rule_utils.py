"""Unit tests for rule_utils module."""

import pytest

from agent_control_server.services.rule_utils import (
    is_agent_scoped,
    parse_rule_ref_full,
...
class TestParseRuleRefFull:
    """Tests for parse_rule_ref_full function (full three-way parsing)."""

    def test_builtin_rule(self) -> None:
        """Given a built-in rule, when parsing full, then type is builtin."""
        # When
        result = parse_rule_ref_full("regex")
    ...
    def test_external_rule(self) -> None:
        """Given an external rule, when parsing full, then type is external."""
        # When
        result = parse_rule_ref_full("galileo.luna")
    ...
    def test_agent_scoped_rule(self) -> None:
        """Given an agent-scoped rule, when parsing full, then type is agent."""
        # When
        result = parse_rule_ref_full("my-agent:pii-detector")
    ...
    def test_external_with_nested_path(self) -> None:
        """Given an external rule with nested path, when parsing, splits on first dot."""
        # When
        result = parse_rule_ref_full("acme.safety.toxicity")
    ...
    def test_agent_scoped_with_dot_in_name(self) -> None:
        ...
        result = parse_rule_ref_full("my-agent:vendor.eval")
