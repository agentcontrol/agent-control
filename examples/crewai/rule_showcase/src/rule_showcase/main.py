"""
CrewAI Data Analyst with All Four Built-in Rules.

Demonstrates every built-in Agent Control rule in a realistic
...
PREREQUISITE:
    ...
    Then run this example:
        ...
        $ uv run --active rule_showcase

Scenarios:
    1. Safe SELECT query            -> SQL rule ALLOWS
    2. DROP TABLE injection         -> SQL rule DENIES
    3. Query without LIMIT          -> SQL rule DENIES
    4. Query sensitive table        -> LIST rule DENIES
    5. Query returns PII            -> REGEX rule DENIES (post-execution)
    6. Valid analysis request       -> JSON rule ALLOWS
    7. Missing required fields      -> JSON rule DENIES
    8. Missing purpose field        -> JSON rule STEERS (then allowed)
"""
...
from rule_showcase.crew import RuleShowcaseCrew
...
agent_control.init(
    ...
    agent_description="CrewAI data analyst with all rule types",
...
def run_direct_test(title, rule, tool_fn, input_data, expected):
...
def main():
    print("=" * 60)
    print("  CrewAI Data Analyst - Rule Showcase")
    print("  All 4 Built-in Rules: SQL, LIST, REGEX, JSON")
    print("=" * 60)
    print()
    ...
    # ════════════════════════════════════════════════════════════════
    #  REGEX RULE SCENARIOS
    # ════════════════════════════════════════════════════════════════
    print("\n" + "#" * 60)
    print("  PART 3: REGEX RULE")
