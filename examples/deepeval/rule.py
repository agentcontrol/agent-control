"""DeepEval GEval rule implementation.

This rule demonstrates how to extend the base Rule class to integrate
...
from config import DeepEvalRuleConfig
...
@register_rule
class DeepEvalRule(Rule[DeepEvalRuleConfig]):
    """DeepEval GEval rule for custom LLM-based evaluations.

    This rule uses DeepEval's GEval metric, which leverages LLM-as-a-judge
    ...
    Example:
        ```python
        from examples.deepeval import DeepEvalRule, DeepEvalRuleConfig
    ...
    metadata = RuleMetadata(
        name="deepeval-geval",
        version="1.0.0",
        description="DeepEval GEval custom LLM-based rule",
        requires_api_key=True,
        timeout_ms=30000,
    )
    config_model = DeepEvalRuleConfig
    ...
    def __init__(self, config: DeepEvalRuleConfig) -> None:
        """Initialize DeepEval rule with configuration.

        Args:
            config: Validated DeepEvalRuleConfig instance.
    ...
    async def evaluate(self, data: Any) -> RuleResult:
        ...
        Returns:
            RuleResult with matched status and metadata.
        """
        try:
    ...
    def _parse_metric_result(self) -> RuleResult:
        """Parse GEval metric results into RuleResult.
    ...
    def _handle_error(self, error: Exception) -> RuleResult:
        ...
        Returns:
            RuleResult indicating error state.
        """
